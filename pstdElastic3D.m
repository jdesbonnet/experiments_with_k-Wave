...
%
% DESCRIPTION:
%       pstdElastic3D simulates the time-domain propagation of elastic
%       waves through a three-dimensional homogeneous or heterogeneous
%       medium given four input structures: kgrid, medium, source, and
%       sensor. The computation is based on a pseudospectral time domain
%       model which accounts for viscoelastic absorption and heterogeneous
%       material parameters. At each time-step (defined by kgrid.t_array),
%       the wavefield parameters at the positions defined by sensor.mask
%       are recorded and stored. If kgrid.t_array is set to 'auto', this
%       array is automatically generated using makeTime. An anisotropic
%       absorbing boundary layer called a perfectly matched layer (PML) is
...
%       which case it is applied to all source elements), or a matrix of
%       time series following the source elements using MATLAB's standard
...
%       mask.          
%
%       pstdElastic3D may also be used for time reversal image
...
% using linear interpolation, where sgx  = (x + dx/2, y, z), 
% sgy  = (x, y + dy/2, z) and sgz = (x, y, z + dz/2)
...
% interpolation where sgxy = (x + dx/2, y + dy/2, z), etc
if numDim(mu) == 3 && use_sg
    
    % mu is heterogeneous and staggered grids are used
    mu_sgxy  = interpn(kgrid.x, kgrid.y, kgrid.z, mu, kgrid.x + kgrid.dx/2, kgrid.y + kgrid.dy/2, kgrid.z, '*linear');
    mu_sgxz  = interpn(kgrid.x, kgrid.y, kgrid.z, mu, kgrid.x + kgrid.dx/2, kgrid.y, kgrid.z + kgrid.dz/2, '*linear');
    mu_sgyz  = interpn(kgrid.x, kgrid.y, kgrid.z, mu, kgrid.x, kgrid.y + kgrid.dy/2, kgrid.z + kgrid.dz/2, '*linear');
...
if kelvin_voigt_model
    if numDim(eta) == 3 && use_sg
        ...
        eta_sgxy  = interpn(kgrid.x, kgrid.y, kgrid.z, eta, kgrid.x + kgrid.dx/2, kgrid.y + kgrid.dy/2, kgrid.z, '*linear');
        eta_sgxz  = interpn(kgrid.x, kgrid.y, kgrid.z, eta, kgrid.x + kgrid.dx/2, kgrid.y, kgrid.z + kgrid.dz/2, '*linear');
        eta_sgyz  = interpn(kgrid.x, kgrid.y, kgrid.z, eta, kgrid.x, kgrid.y + kgrid.dy/2, kgrid.z + kgrid.dz/2, '*linear');
...
for t_index = index_start:index_step:index_end
    ...
    if uz_source >= t_index
        if strcmp(source.u_mode, 'dirichlet')
            % enforce the source values as a dirichlet boundary condition        
            uz_split_z(u_source_index) = source.uz(:, t_index);
        else
            % add the source values to the existing field values 
            uz_split_z(u_source_index) = uz_split_z(u_source_index) + source.uz(:, t_index);
        end
    end    
    ...
    if kelvin_voigt_model
        ...
        % model
        dduxdxdt = real(ifft( bsxfun(@times, ddx_k_shift_neg, fft( (dsxxdx + dsxydy + dsxzdz)./rho0_sgx, [], 1 )), [], 1));   
        dduydydt = real(ifft( bsxfun(@times, ddy_k_shift_neg, fft( (dsxydx + dsyydy + dsyzdz)./rho0_sgy, [], 2 )), [], 2));
        dduzdzdt = real(ifft( bsxfun(@times, ddz_k_shift_neg, fft( (dsxzdx + dsyzdy + dszzdz)./rho0_sgz, [], 3 )), [], 3));
        
        dduxdydt = real(ifft( bsxfun(@times, ddy_k_shift_pos, fft( (dsxxdx + dsxydy + dsxzdz)./rho0_sgx, [], 2 )), [], 2));
        dduydxdt = real(ifft( bsxfun(@times, ddx_k_shift_pos, fft( (dsxydx + dsyydy + dsyzdz)./rho0_sgy, [], 1 )), [], 1)); 
        
        dduxdzdt = real(ifft( bsxfun(@times, ddz_k_shift_pos, fft( (dsxxdx + dsxydy + dsxzdz)./rho0_sgx, [], 3 )), [], 3));
        dduzdxdt = real(ifft( bsxfun(@times, ddx_k_shift_pos, fft( (dsxzdx + dsyzdy + dszzdz)./rho0_sgz, [], 1 )), [], 1));
        
        dduydzdt = real(ifft( bsxfun(@times, ddz_k_shift_pos, fft( (dsxydx + dsyydy + dsyzdz)./rho0_sgy, [], 3 )), [], 3));
        dduzdydt = real(ifft( bsxfun(@times, ddy_k_shift_pos, fft( (dsxzdx + dsyzdy + dszzdz)./rho0_sgz, [], 2 )), [], 2));
    ...
    if sxx_source >= t_index
        if strcmp(source.s_mode, 'dirichlet')
            ...
            sxx_split_z(s_source_index) = source.sxx(:, t_index);
        else
            % add the source values to the existing field values 
            ...
            sxx_split_z(s_source_index) = sxx_split_z(s_source_index) + source.sxx(:, t_index);
        end
    end
    if syy_source >= t_index
        if strcmp(source.s_mode, 'dirichlet')
            ...
            syy_split_z(s_source_index) = source.syy(:, t_index);
        else
            % add the source values to the existing field values 
            ...
            syy_split_z(s_source_index) = syy_split_z(s_source_index) + source.syy(:, t_index);
        end
    end
    if szz_source >= t_index
        if strcmp(source.s_mode, 'dirichlet')
            % enforce the source values as a dirichlet boundary condition
            szz_split_x(s_source_index) = source.szz(:, t_index);
            szz_split_y(s_source_index) = source.szz(:, t_index);
            szz_split_z(s_source_index) = source.szz(:, t_index);
        else
            % add the source values to the existing field values 
            szz_split_x(s_source_index) = szz_split_x(s_source_index) + source.szz(:, t_index);
            szz_split_y(s_source_index) = szz_split_y(s_source_index) + source.szz(:, t_index);
            szz_split_z(s_source_index) = szz_split_z(s_source_index) + source.szz(:, t_index);
        end
    end    
    ...
    if sxz_source >= t_index
        if strcmp(source.s_mode, 'dirichlet')
            % enforce the source values as a dirichlet boundary condition
            sxz_split_x(s_source_index) = source.sxz(:, t_index);
            sxz_split_z(s_source_index) = source.sxz(:, t_index);
        else
            % add the source values to the existing field values 
            sxz_split_x(s_source_index) = sxz_split_x(s_source_index) + source.sxz(:, t_index);
            sxz_split_z(s_source_index) = sxz_split_z(s_source_index) + source.sxz(:, t_index);
        end
    end  
    if syz_source >= t_index
        if strcmp(source.s_mode, 'dirichlet')
            % enforce the source values as a dirichlet boundary condition
            syz_split_y(s_source_index) = source.syz(:, t_index);
            syz_split_z(s_source_index) = source.syz(:, t_index);
        else
            % add the source values to the existing field values 
            syz_split_y(s_source_index) = syz_split_y(s_source_index) + source.syz(:, t_index);
            syz_split_z(s_source_index) = syz_split_z(s_source_index) + source.syz(:, t_index);
        end
    end     
...
% plot normal stress
subplot(2, 3, 1), imagesc(y_vec, x_vec, squeeze(s_normal_plot(:, :, round(end/2))), plot_scale(1:2));
title('Normal Stress (x-y plane)'), axis image;

subplot(2, 3, 2), imagesc(z_vec, x_vec, squeeze(s_normal_plot(:, round(end/2), :)), plot_scale(1:2));
title('Normal Stress  (x-z plane)'), axis image;

subplot(2, 3, 3), imagesc(z_vec, y_vec, squeeze(s_normal_plot(round(end/2), :, :)), plot_scale(1:2));
title('Normal Stress  (y-z plane)'), axis image;

% plot shear stress
subplot(2, 3, 4), imagesc(y_vec, x_vec, squeeze(s_shear_plot(:, :, round(end/2))), plot_scale(end-1:end));
title('Shear Stress (x-y plane)'), axis image;

subplot(2, 3, 5), imagesc(z_vec, x_vec, squeeze(s_shear_plot(:, round(end/2), :)), plot_scale(end-1:end));
title('Shear Stress (x-z plane)'), axis image;

subplot(2, 3, 6), imagesc(z_vec, y_vec, squeeze(s_shear_plot(round(end/2), :, :)), plot_scale(end-1:end));
